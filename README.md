...
5. 运行 `./scripts/create_topics.sh`，再将生成器以 `--kafka-bootstrap kafka:29092` 运行，把事件推送至 `telemetry-raw`。
6. 运行 `./scripts/submit_spark.sh`；作业会把有效事件写入 Parquet，并将无效事件写到 `telemetry-quarantine`。
