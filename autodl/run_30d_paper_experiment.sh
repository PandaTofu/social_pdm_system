...
CONFIG_KS_THRESHOLD="$("$PYTHON_BIN" -c 'import json, sys; print(json.load(open(sys.argv[1], encoding="utf-8")).get("drift_detection", {}).get("ks_threshold", 0.10))' "$CONFIG")"
CONFIG_KS_ALPHA="$("$PYTHON_BIN" -c 'import json, sys; print(json.load(open(sys.argv[1], encoding="utf-8")).get("drift_detection", {}).get("alpha", 0.05))' "$CONFIG")"
KS_THRESHOLD="${PDM_KS_THRESHOLD:-$CONFIG_KS_THRESHOLD}"
KS_ALPHA="${PDM_KS_ALPHA:-$CONFIG_KS_ALPHA}"
